...
(function () {
    ...
    var DEFAULT_COVER = 'https://images.unsplash.com/photo-1511379938547-c1f69419868d?w=600&q=80';
    var picker = null;
    ...
    function enhanceRows() {
        document.querySelectorAll('.song-row').forEach(function (row) {
            ...
            var button = document.createElement('button'); button.type = 'button'; button.className = 'song-action-btn playlist-more-button'; button.title = 'Действия с треком'; button.innerHTML = '<span class="material-symbols-outlined">more_vert</span>';
            button.addEventListener('click', function (event) { event.stopPropagation(); openPicker(trackFromRow(row)); }); row.appendChild(button);
        });
    }

    var style = document.createElement('style');
    style.textContent = '.playlist-more-button{display:none!important}.song-row:hover .playlist-more-button,.playlist-more-button:focus{display:flex!important}.playlist-picker-backdrop{position:fixed;inset:0;z-index:2000;background:rgba(0,0,0,.62);display:flex;align-items:center;justify-content:center;padding:18px}.playlist-picker{display:grid;grid-template-columns:42% 58%;width:min(780px,100%);max-height:88vh;overflow:hidden;background:#292929;border-radius:10px;box-shadow:0 20px 70px #000}.playlist-picker-left{padding:14px;border-right:1px solid rgba(255,255,255,.12);overflow:auto}.playlist-picker-right{position:relative;padding:32px 24px}.playlist-search{width:100%;padding:13px;background:#404040;border:0;border-radius:7px;color:#fff;font-size:16px}.playlist-create{width:100%;padding:14px 4px;background:none;border:0;border-bottom:1px solid rgba(255,255,255,.12);color:#fff;text-align:left;font-size:16px;cursor:pointer}.playlist-choice{display:flex;gap:12px;align-items:center;width:100%;padding:11px 8px;background:none;border:0;border-radius:6px;color:#fff;text-align:left;cursor:pointer}.playlist-choice:hover{background:#3b3b3b}.playlist-choice small{display:block;color:var(--text-muted);margin-top:3px}.playlist-picker-close{position:absolute;right:14px;top:8px;border:0;background:none;color:#fff;font-size:28px;cursor:pointer}.playlist-selected-track{color:var(--text-muted)}.playlist-toast{position:fixed;left:50%;bottom:calc(var(--player-height) + 18px);z-index:3000;transform:translate(-50%,18px);opacity:0;background:#2d1a55;color:#fff;border:1px solid #854dff;border-radius:8px;padding:12px 18px;transition:.2s;pointer-events:none}.playlist-toast.visible{opacity:1;transform:translate(-50%,0)}.user-playlist-card{cursor:pointer}.playlist-detail-view{position:fixed;inset:0 0 var(--player-height) 0;z-index:1500;overflow:auto;background:#151515;color:#fff;padding-bottom:40px}.playlist-detail-header{position:relative;min-height:390px;padding:70px 7%;display:flex;align-items:flex-end;gap:35px;background:linear-gradient(120deg,#444,#171717)}.playlist-detail-cover{width:260px;height:260px;object-fit:cover;box-shadow:0 15px 35px #000}.playlist-detail-label{font-size:16px}.playlist-detail-header h1{font-size:clamp(38px,7vw,90px);margin:18px 0}.playlist-back{position:absolute;left:24px;top:20px;background:none;border:0;color:#fff;font-size:16px;cursor:pointer}.playlist-detail-more{position:absolute;right:28px;top:20px;border:0;background:none;color:#fff;cursor:pointer}.playlist-add-track,.playlist-edit{border:0;border-radius:22px;padding:10px 17px;margin-right:8px;background:#444;color:#fff;cursor:pointer}.playlist-detail-list{padding:25px 7%}.playlist-detail-row{display:flex;align-items:center;gap:16px;padding:10px;border-bottom:1px solid rgba(255,255,255,.08);cursor:pointer}.playlist-detail-row img{width:48px;height:48px;object-fit:cover;border-radius:5px}.playlist-detail-row div{flex:1}.playlist-detail-row small{display:block;color:#aaa;margin-top:4px}.playlist-remove-track{background:none;border:0;color:#aaa;font-size:24px;cursor:pointer}.playlist-detail-menu{position:fixed;z-index:2500;background:#2b2b2b;box-shadow:0 10px 30px #000;border-radius:6px;padding:5px;min-width:190px}.playlist-detail-menu button{display:block;width:100%;padding:12px;background:none;border:0;color:#fff;text-align:left;cursor:pointer}.playlist-detail-menu button:hover{background:#444}@media(max-width:600px){.playlist-picker{grid-template-columns:1fr}.playlist-picker-right{display:none}.playlist-detail-header{min-height:340px;padding:50px 22px 28px;display:block}.playlist-detail-cover{width:150px;height:150px}.playlist-detail-header h1{font-size:42px}.playlist-detail-list{padding:20px 12px}}';
    document.head.appendChild(style);

    document.addEventListener('DOMContentLoaded', function () {
        renderCards(); enhanceRows();
        var observer = new MutationObserver(enhanceRows); observer.observe(document.body, { childList: true, subtree: true });
    });
})();
